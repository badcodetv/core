...
| `docs/suno-gpt/` | Suno toolkit — **🔑 All new Suno work is v6, in the new UI (v5.5 retired 2026-09-09, archived in `docs/suno-gpt/archive/v5.5-era.md`): [`files/suno-v6.md`](./docs/suno-gpt/files/suno-v6.md) wins on any model/control question, and its §3 is the ladder for fixing one word / one bar without regenerating** — prompting (operating procedure + reference files) **and** [`automation.md`](./docs/suno-gpt/automation.md), the create-page DOM map + the five silent traps + the pair/naming/workspace/no-download protocol | …you're turning a song idea into a Suno prompt, or driving Suno from code |
| `docs/ai-studio/` | Google AI Studio **voice** toolkit — [`README.md`](./docs/ai-studio/README.md): the Composer UI map (Scene / Sample Context / Voice Direction), Google's four-part prompt structure, the **5,000-voice library with an Accent filter** (the docs say 30 — the UI is the truth), the 200+ audio tags, limits and traps. 🔑 **First engine that sounded human on take one** | …you're generating spoken narration, or a voice sounds like AI |
...
| `docs/video-fx/` | Which tool for the job — **[`hybrid-method.md`](./docs/video-fx/hybrid-method.md) (the lanes are LAYERS: Veo animates the world camera-locked, Premiere moves the camera, ffmpeg does what must be exact — and 8s is not a limit)**, lane choice (Flow / sourcing / ffmpeg / Premiere), **[`ffmpeg-catalogue.md`](./docs/video-fx/ffmpeg-catalogue.md) (319 video filters + the 133-plugin frei0r shelf — the twin of Premiere's catalogue; asked what effects we have, list BOTH)**, the free stack, the 20 research briefs, [`delivery.md`](./docs/video-fx/delivery.md) (**the QC gate before anything ships** — run `scripts/delivery-qc.sh`) and [`footage-sources.md`](./docs/video-fx/footage-sources.md) (68 footage sources, tiered green/amber/red; **the £100/£200-per-film footage budget** and the credits ruling live there). **No paid plugins; footage is free-first with a tiny licence budget as a stop gate** | …you're choosing an effect, wondering if a tool exists, looking for real footage, or about to upload |
...
| `docs/cinematography/` | Shot-craft toolkit (`shot-craft` skill) — **what a shot should BE**, tool-agnostic and built to outlive Flow: [`symptoms.md`](./docs/cinematography/symptoms.md) (**start here** when something looks flat / boring / "like AI" and you don't know why), [`principles.md`](./docs/cinematography/principles.md) (26 graded principles, 3 gates, 8 house rulings, 1 **open** ruling), [`frame.md`](./docs/cinematography/frame.md) (composition, depth, light, colour, lens), [`motion-and-cutting.md`](./docs/cinematography/motion-and-cutting.md) (**§4 = re-cutting footage that already exists**), [`stills.md`](./docs/cinematography/stills.md), [`registers.md`](./docs/cinematography/registers.md) (the monumental register **and its problem**), [`evidence.md`](./docs/cinematography/evidence.md) (grades + the never-cite list) | …you're designing a shot, judging one, or re-cutting a scene |
| `.claude/skills/` | `new-idea`, `new-marketing-idea`, `new-story`, `suno-prompt`, `suno-automation`, `make-comic`, `edit-panel`, `animate-slide`, `music-video-short`, `new-image`, `badcode-art-direction`, `flow-prompt`, `flow-automation`, `video-fx`, `premiere-automation`, `find-footage`, `story-craft`, `shot-craft` — orchestrators for parking an idea, capturing a marketing/distribution play, story capture, Suno prompting, driving Suno from code, the full idea→comic pipeline, editing an existing panel image, animating a finished panel, the full idea→short-form music-video pipeline (Suno track + Flow clips + edit plan), standalone brand imagery, the BadCode comic register, Google Flow prompt craft, driving the Flow browser, choosing which tool and which effect (plus the delivery gate), driving Adobe Premiere Pro, sourcing real footage we are allowed to publish, making a story grip (the research-backed craft + the adversarial review pass), and designing or diagnosing a shot | …you're capturing an idea or marketing play, developing a story, making a track, automating Suno, building a comic, editing a panel, animating a slide, making a short, making a brand image, writing a Flow prompt, deciding how to achieve an effect or whether a file is ready to upload, cutting video in Premiere, looking for archive/stock footage, asking whether a story engages, or working out why a shot or a cut is flat |
...
- **Decide how to achieve an effect — and whether the file is ready to ship:** run the
  ...
  **Premiere** is where it becomes an edit, and **After Effects is not installed**. Then it routes
  ...
  Knowledge base: [`docs/video-fx/`](./docs/video-fx/README.md). 🔑 **Start at
  [`hybrid-method.md`](./docs/video-fx/hybrid-method.md) — ruled 2026-08-26, the lanes are *layers*,
  not alternatives.** Veo animates the world with its camera **locked** (its regeneration bug is caused
  by camera translation, so locking the camera stops it firing); **Premiere moves the camera** over the
  finished clip, rigid by construction and exactly eased; **ffmpeg** does what must be exact — and it is
  the only lane that can write on-screen text, since Premiere's API cannot set a string at all. The same
  file carries the standing rule that **eight seconds is never a reason to shorten a beat** — chain the
  last frame of one clip into the first of the next, and never pin an `endImage`, which morphs.
  **Two more facts save the most time:**
...
- **Find real footage (and check we may publish it):** run the **`find-footage`** skill
  ...
  **68 sources tiered 🟢/🟡/🔴**, counts dated, with an end-to-end runbook from query to a
  ...
  a blocker, which opens the whole UK government-document route. Record the exact credit string and
  never ship the clip without it. ShareAlike, non-commercial and political-use clauses still bite.
  🔴 **And a search hit is not an item** — archive.org's index outlives its files; `{}` from
  `/metadata/` with HTTP 200 means gone, and a quarter of FedFlix is.
- **Design a shot, or work out why one is flat:** run the **`shot-craft`** skill
  (`.claude/skills/shot-craft/`). The **judgement layer for pictures** — what the shot should
  *be*, decided before any tool is asked for anything, and deliberately **tool-agnostic** so it
  outlives Flow. Reach for it on "what's a more interesting shot", "how should we frame this",
  "this feels flat / boring / like AI", "give me a better angle", "let's discuss the
  cinematography for <scene>", or "an alternate way of cutting <scene>". Three modes: **Design**
  (a shot spec, which `flow-prompt` then phrases), **Diagnose** (start at
  [`symptoms.md`](./docs/cinematography/symptoms.md) — the complaint in ordinary words → cause →
  fix), and **Re-cut** (the clips exist and the cut is flat — jobs not clips, then order, then
  duration, *then* generate). Knowledge base: [`docs/cinematography/`](./docs/cinematography/README.md),
  distilled from a 20-brief sweep with two adversarial critic passes. **Three facts save the most
  time:** **darkness reads as depth only when one small region inside it is deliberately
  brighter** — uniform near-black reads as a broken file, not as mood, and there is a perceptual
  mechanism for it (lightness anchoring), which makes it the fix for our register's central
  problem; **every monumental shot must carry a visible cost** — wreckage, exhaustion, a body
  that failed — because scale-and-symmetry without one reads as *celebration* of the power it
  means to indict, and meaning it ironically demonstrably does not protect you (Verhoeven,
  Harron); and **a scene is a small argument, so every shot is a step in it or it is
  decoration** — most "this cut is flat" notes are really "these shots do not step."
  🔴 **One ruling is open:** whether our near-black monumental register is right for a
  working-class UK reader is unproven in both directions — see `principles.md` §R1, and don't
  call it settled anywhere.
