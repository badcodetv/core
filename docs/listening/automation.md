...
> 🟡 **Status 2026-09-11: PARTIAL.** Mapped live up to the file upload, then stopped at the plan's
> stop gate (2): AI Studio saves every uploaded file to the account's Google Drive, even in a
> Temporary chat, with no off switch found (§4, Trap 1). Nothing has been uploaded. The upload
> signal, the running/done signal and the reply selector are ⬜ until Kai rules.
...
| **Attach** | hidden `input.file-input[type="file"]` inside `ms-add-media-button` (`multiple`, accepts `audio/*`, `.mp3`, `.wav`, …) — set it directly, never the OS chooser. Visible trigger: `button[data-test-id="add-media-button"]` ("Insert images, videos, audio, or files") | present; **not yet used** (gate) |
| **Prompt box** | `textarea[aria-label="Enter a prompt"]` | present |
| **Run** | `ms-run-button button[type="submit"]` (text `Run Ctrl ↵`); `aria-disabled="true"` while the prompt is empty | present |
...
| Upload finished · running/done · last model turn | ⬜ | not mapped — needs an upload (gate) |
...
not marked Paid.** Whether it accepts audio is ⬜ (gate).
...
| Uploads are saved to Drive even in a Temporary chat | ✅ 2026-09-11 (the page says so; no upload made) |
| A Pro model accepts audio and replies | ⬜ gate |
| Upload-finished signal, run/done signal, reply selector, `innerText` keeps headings | ⬜ gate |
| Rate-limit / error wording | ⬜ none seen |
