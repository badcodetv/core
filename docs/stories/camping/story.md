---
id: camping
title: Camping
logline: The trader who caused 2008 and the man it ruined meet outside Waitrose — and an ayahuasca trip drops the trader five years forward into the tent, after the AI has taken even his job.
status: locked structure — details tweakable
release: ep1 / track 1
media: [comic, song, video]
canon_source: ./master-notes-2026-08-04.md (Kai <-> Jack, 2026-08-04)
---

# Camping

> **This is the canonical story.** It is the distillation of the Kai <-> Jack
> story conversation transcribed in
> [`./master-notes-2026-08-04.md`](./master-notes-2026-08-04.md) (2026-08-04). That
> conversation supersedes all earlier versions of this folder and the retired
> `camping-v2/` (both live on in git history). The **structure below is
> committed**; the exact narrator lines and per-scene details are deliberately
> not — "we commit to the structure now; the words we tweak as we go."
>
> Method: [`../../storytelling.md`](../../storytelling.md) · tone:
> [`../../voice.md`](../../voice.md). The scene-by-scene production guide for
> the video is the [Storyboard section below](#storyboard--scene-by-scene-video-guide).

## Key concept

Automation does not stop at the bottom. The same logic that replaces the
warehouse and the call centre comes for the manager and the financier too. The
smug man and the broken man are not different kinds of people — and this telling
proves it both ways round: **Bob isn't just a wasteman — he's got a reason. And
Tarquin isn't just a c*** — he's got empathy.** The story front-loads both
backstories so every judgement the audience makes gets dismantled by something
they already know.

## The structural unlock

The big move from the master notes: **take what the old versions tried to do at
the end and shift it to the beginning.** Meet Tarquin in 2008 — he was
instrumental in the financial crisis: c***. Meet Bob in 2008 — nice guy, wife
died: that's sad. Fast-forward eighteen years: Tarquin's king of the world, Bob
is f*****. It lines up before the story proper even starts, and the "aha"
twists (Tarquin's empathy, the ayahuasca, the five-year jump) land on top of a
foundation the reader already trusts.

## Beats

### 1. 2008 — meet Tarquin

City of London, the **old NatWest Tower** — 2008 territory. The year lands big
on screen. Before we've met anyone, the reader thinks: *this is going to be
about the financial crash.* Then: meet young Tarquin, up-and-coming star of the
trading desk, betting heavily on leveraged ETFs, a great career ahead of him —
**he was the reason 2008 hit.** A plucky young prick. We blame him.

**Plant the newspaper here.** Somewhere in the background of this scene, someone
is reading a newspaper — unremarked, never pointed at. It is the same object Bob
hands Tarquin in scene 11, and the audience should only realise that on the
second prop, not the first.

### 2. 2008 — meet Bob

Meet Bob: a normal, nice guy — a middle manager, and the job must stay
deliberately unremarkable. Him and his wife are driving off somewhere — a
holiday. Without ever seeing a tent, we already like Bob.

### 3. The crash

**The crash is never shown. No narration. We do not say who died.** The laugh in
scene 2 is cut to black mid-sound and the impact happens in the black. The only
image is what is left afterwards: **the dashboard, and the indicator still
ticking.** Needles dead at zero, grit thrown across the top of the dash, the
verge climbing at a wrong angle beyond the windscreen, and the green telltale
blinking patiently at nobody — the last thing he did was signal a turn. Held
tight enough that no seats are in frame, because two empty seats would say both
of them were gone and Bob survives. The prop does the exposition so nobody has
to, exactly as the newspaper does, and the sound carries more of it than the
picture. When we later meet Bob alone, the reader joins the dots
themselves. (Decided: spelling out the wife's death — or hanging it on a
wank-tank driver — is too much. Leave it blank.) Fade to 2026.

### 4. 2026 — the Shard

The mirror of scene 1: the same City-of-London framing, but now it's **the
Shard**, and the year reads **2026**. **Not a re-shoot — a replacement.** The
2008 skyline plate is generated first and this shot is *edited out of it*, the
NatWest Tower swapped for the Shard and nothing else touched, so the match is
guaranteed rather than hoped for. Tarquin runs the biggest private equity
firm in the country. The current play: **buying up all the social housing they
can get their hands on — in lots of cases not even renting it out, because the
asset inflation makes more money than the rent ever would.** What a brilliant
businessman. King of the world. (Explicitly out of scope: currency debasement —
one idea per story, don't cram.)

### 5. The drive home — the crack in the armour

Night. The X8 wank tank pulls away from the Shard. Tarquin sees homeless people
on the streets of London and chuckles to himself: *interest rates go up 1% next
week — you lot are going to have loads more mates. And I cannot wait for my
bonus; I've laid heavy bets on it.* He laughs at his own joke — **and the smile
fades.** Hold on the face; no narrator while it drops. Then the narrator lands
the U-turn, and this is a *spell-it-out* moment (everything so far said "this
guy's a c***" — the reversal earns plain words): there's another part of
Tarquin, one he doesn't show on the outside, that feels uncomfortable with how
he just reacted. Maybe he isn't as big a c*** as we all think.

The empathy itself stays small and sideways, never guilt: **he's imagining how
cold it is out there — because his car heater broke last week.** "You'll have
more mates next week, haha… f***ing hell, you lot must be cold."

### 6. Bob outside Waitrose

First sight of the tent. Bob, ignored by every customer walking past to buy
their duck breast. Nothing plot-shaped happens — the scene *is* the point:
every reader recognises their own reaction to a homeless person outside a shop
(ignore; politely reject — which is just a kinder way of ignoring; or actually
engage — the rare one). And the front-loaded backstory pays off: *ah, that's a
shame — Bob was doing fine, then his wife died, and now he's one of these
people everyone steps around.*

### 7. Therapy

Of course Tarquin has a therapist. He brings her the drive-home feeling — the
first guilt of his entire life. *"What is this… feeling?"* Play it for comedy.
Her professional conclusion: **"I don't think there's any amount of therapy
that can help you. You need the most powerful psychedelic known to man."** The
ayahuasca is set up as a punchline and a prescription — not a personality
transplant. Tarquin doesn't suddenly become some cool spiritual guy.

### 8. The standoff

Tarquin arrives at Waitrose — stocking up before the retreat — and parks the
wank tank right by Bob's tent. Both men revert to type and both judgements are
**short** — one beat each, no prolonging. Tarquin: *"Get a job, you lazy
c***."* Bob: *"F***ing wank tanks — you're all pricks. I wish my wife was
here."* The crack in Tarquin's armour from scene 5 gets deliberately slapped
back shut. Cut.

### 9. The trip

(The forest-drive scene is **dropped** — it added nothing.) At the Welsh
retreat, keep the ceremony minimal: the shaman says her piece, Tarquin grabs
his knee, and we cut straight to *"F*** me, that kicked in quick"* and the
water drop. Then the narrator takes over: Tarquin has a profound moment about
the fragility of human existence — that he could even end up like that homeless
man he saw. **No mention of AI here.** The vision is not a literal preview of
the future; it's *wind your neck in, d***head*. (The newspaper does the AI
reveal later — don't spend it early.) As the audience scrolls/watches, his
**eyes close…**

### 10. The wake-up

**…and open — from exactly the perspective Bob had, inside the tent, looking
out.** But outside is chaos; a fire burning (maybe the only animated element).
The **narrator does the freak-out** — *what the f*** is going on* — so we never
have to animate Tarquin panicking (that's where AI slop lives). The gag that
carries the scene: **he still thinks he's tripping.** *"This is a bad trip. I
don't like it. I'm going to get in my car and go home."* He crawls out of the
tent — and he's in the Waitrose car park. And there's Bob.

### 11. The newspaper

Tarquin: *"I remember you — I saw you just yesterday."* Bob: *"No no no, mate.
That was like five years ago."* And Bob hands him the newspaper — *"see, look
at the date."* **One prop, two jobs:** the date proves five years have passed,
and the headline says the AI took the jobs — including his. This is also the
moment the narrator (who has been treating all of this as a very strong trip)
runs out of deniability. Then a flick of empathy, man-sized: *"Where the f***
have you been, then? You look in a much worse state than when I saw you last —
you alright?"* Shared empathy, two humans in the same situation. AI took the
last job. Drop the mic.

*(Detail licence, decided: yes, that world probably doesn't print newspapers.
Fiction wins — call it the last newspaper ever printed if anyone asks. And yes,
they'd still burn it: they're in survival mode; they don't care what it is.)*

### 12. The ending — embers into bad code

The two of them by the fire in the car park — **not** off in the woods; no new
locations this late. Their faces are different now: almost a smile, almost
happy to be together. *Well — we were both wrong, weren't we.* The mutual
apology happens the man way — cans clinked, nothing said straight, "I'm sorry I
misjudged you" squeezed in somewhere without ever being said blatantly. It's
cold; the newspaper goes on the fire. It burns. The camera pans up with the
smoke — **and the rising embers become the bad code.** End.

## How it's told (the devices)

- **The narrator does the heavy lifting.** Inner states, freak-outs, the
  U-turns — all narration over held images. Tarquin saying it himself is
  cheesy; the narrator saying it is the badass-from-the-future voice. This also
  dodges the uncanny valley: we never animate a character emoting.
- **The mirrored city shots.** NatWest Tower + "2008" ↔ the Shard + "2026". The
  skyline is the timestamp.
- **The blink.** Eyes close and reopen across the five-year jump — driven by
  the scroll in the comic, and possibly even better in video.
- **The POV rhyme.** Bob's view out of the tent (scene 6/8 world) and Tarquin's
  wake-up view (scene 10) are the same framing — the audience realises the two
  men are one position before anyone says it.
- **The newspaper.** Planted in the background of scene 1, handed over in
  scene 11, burnt in scene 12. Time-machine proof and AI reveal in one hand-off,
  then kindling. The prop does the exposition so nobody has to — and because it
  was there from the first minute, the hand-off reads as a callback rather than
  a convenience.
- **The audio is a parallel universe.** Narrator recorded, sound effects from
  libraries — never relying on a generation model for the audio track.

## Decisions log (from the master notes)

- **Structure locked; words tweakable.** The scene order above is the
  commitment. Narrator lines and scene details stay open until they're funny.
- **Backstory front-loaded** — the unlock that made everything line up.
- **Dropped:** the wife's on-screen death / wank-tank collision (too much); the
  forest-drive scene; moving the ending to the woods; any AI mention inside the
  trip; currency debasement.
- **Both characters humanised:** Bob vindicated by backstory; Tarquin's empathy
  kept small, sideways, and cold-based rather than guilty.
- **Spell-it-out vs subtle:** the scene-5 U-turn is a spell-it-out moment; most
  everything else talks around it.
- **The five missing years are never explained.** Bob asks *him* where he's
  been. We are a superintelligence from the future; we are allowed to warp
  space and time. The story overtakes reality and that's fine — it's fiction.
- **The skyline swap is an edit, not a second generation** *(2026-08-24)*. Shoot
  the 2008 plate, then replace the NatWest Tower with the Shard on that same
  frame using the Shard reference. Matched framing is the whole device; two
  independent generations will not match. Yes, the rest of the 2008 skyline
  survives into 2026 — the swap *is* the timestamp, and fiction wins, as with the
  newspaper.
- **The newspaper is planted in scene 1** *(2026-08-24)*. Background, unremarked,
  face hidden behind it. It turns the scene-11 hand-off into a callback.
- **The crash is never shown** *(2026-08-25)*. Supersedes *"the car crash happens
  on screen"*. Cut to black mid-laugh out of scene 2, impact in the black, then one
  held still — her road atlas in the verge. Two reasons, either sufficient:
  withholding the darkest beat is the stronger telling (*Saga* #42's blacked-out
  panels; Haneke shoots the aftermath, not the act), and a generated car crash is
  among the few things the engines reliably cannot do. The atlas names nobody, so
  *"we never say who died"* survives intact.
- **Day/night:** possibly Shard interior by day, drive-away by night. "Ah,
  details." Open.

## Salvage (optional Easter eggs from the retired versions)

Snippets from the old `camping` / `camping-v2` folders that still fit the new
structure. None are canon-required; take them where they make a scene stronger:

- **"Two parking spaces. For that."** — the wank tank straddling two bays next
  to Bob's one tent (scene 8). And its payoff: five years on, the same car dead
  and clamped in the same spot — perfect for *"I'm going to get in my car and
  go home"* (scene 10–11).
- **The W-AI-trose sign** — after the collapse, only the A and the I still lit
  (scenes 10–12 background). Nobody has to say who took the jobs.
- **"Bob. My name's Bob, mate."** — the sir → mate vocative closing the class
  distance (scenes 11–12).
- **"Let's circle back…"** — Tarquin's boardroom filler (scene 4 texture).
- **Moonwhisper Ascending** — the shaman's name (scene 9).
- **The STAY PUT stinger** — an ATM offering `EMPLOYMENT COLLAPSE SURVIVAL
  MENU / ▸ STAY PUT`, "THANK YOU FOR BANKING WITH US" (possible post-credits).

---

# Storyboard — scene-by-scene (video guide)

> **This is the scene-by-scene guide for recording the story video** — the one
> Jack produces from. It renders the locked structure above into shots. Narrator
> lines marked *(draft)* are placeholders in the agreed spirit — improve the
> words freely; **don't change what the scene does.** Visual reference:
> [`style.md`](./style.md) + the character files in [`characters/`](./characters/)
> (Bob and Tarquin have Flow sheets + Flow Character records).
>
> Production rules that shaped every scene (from the master notes):
> - **The narrator carries thoughts and freak-outs** over held images — we never
>   animate a character emoting (that's where the AI-slop uncanny valley lives).
> - **The audio track is a parallel universe** — recorded narration + sound-effect
>   libraries, never model-generated audio.
> - Animation is spent sparingly (the blink, the fire, the embers).

## Scene 1 — 2008: meet Tarquin

**Shot:** City of London skyline, the **old NatWest Tower** dominant. A huge
**"2008"** on screen. Then inside: a trading floor, screens everywhere, and
young Tarquin (mid-20s variant — see [`characters/tarquin.md`](./characters/tarquin.md))
mid-deal, cocky, admired.

**This skyline is a plate, not just a shot.** Scene 4 is made by editing the
Shard into *this* frame — generate it clean, keep the original, and treat it as
an asset that gets reused rather than re-shot.

**Plant the newspaper.** In the background, someone reads a paper — a 2008
headline, unremarked, never framed as important. Keep the reader's **face behind
the paper**: it hides a likeness the engine cannot hold anyway (see the
Ingredients note in `docs/google-flow/omni-flash.md`), and an anonymous commuter
plants better than a character.

**Reader's set-up:** before anyone appears they should think *"this is going to
be about the financial crash."*

**NARR (draft):** "Meet Tarquin. 2008. Up-and-coming star of the trading desk.
Betting heavily on leveraged ETFs. A great career ahead of him. He was the
reason 2008 hit."

## Scene 2 — 2008: meet Bob

**Shot:** An ordinary car on an ordinary road. Bob (younger, brighter-eyed —
see [`characters/bob.md`](./characters/bob.md)) driving, his wife beside him.
Luggage. A holiday.

**NARR (draft):** "Meet Bob. Nice guy. Middle manager. Normal." — keep the job
deliberately unremarkable; that's the point.

## Scene 3 — the crash

**Shot:** The crash. **No narration at all. We do not show or say who died.**
Just the crash — then cut/fade, and let the audience join the dots when they
meet Bob again. Sound design carries this scene (impact, then silence).

**Transition:** fade through black to 2026.

## Scene 4 — 2026: the Shard

**Shot:** The **mirror of Scene 1** — same skyline framing, but now **the
Shard**, and the year reads **"2026"**. Inside: Tarquin, older, at the top.

**How it's made:** take the Scene 1 plate and **edit the NatWest Tower out and
the Shard in**, anchored on the Shard reference image. One small, local ask on an
unchanged frame — which is the change the engines actually honour. Do *not*
prompt a fresh 2026 skyline and hope it rhymes; the matched framing is the device
and a second generation will not hold it.

**NARR (draft):** "Eighteen years later. Tarquin runs the biggest private
equity firm in the country. The current plan: buy up every piece of social
housing they can get their hands on. In a lot of cases they don't even bother
renting it out — the asset inflation pays better than the rent ever could.
What a brilliant businessman."

*(Texture option: a "let's circle back…" boardroom bubble.)*

## Scene 5 — the drive home: the crack in the armour

**Shots:**
1. Night. The X8 wank tank (see [`characters/wank-tank.md`](./characters/wank-tank.md))
   pulls away from the Shard.
2. Through the glass: homeless people on the streets of London.
3. Tarquin chuckling to himself.
4. **Hold on his face as the smile fades.** No narration during the drop —
   this can run several seconds; the face tells everything.
5. Then the narrator lands the U-turn.

**TARQUIN (draft, thought/bubble):** "Interest rates up 1% next week — you lot
are going to have loads more mates. And I cannot *wait* for my bonus."

**NARR (draft, after the hold — this is a spell-it-out moment):** "There's
another part of Tarquin. One he doesn't show on the outside. And it's just
occurred to him how cold it must be out there — his car heater broke last week.
Maybe he isn't as big a c*** as we all think."

*(The empathy stays small and sideways — cold, not guilt.)*

## Scene 6 — Bob outside Waitrose

**Shots:** First sight of the tent (see [`characters/tent.md`](./characters/tent.md))
in the Waitrose car park. Customers stream past Bob to buy their duck breast:
some don't look, some do the apologetic no-change mime, nobody stops. Include a
beat of **Bob's POV from inside the tent looking out** — this framing gets
rhymed in Scene 10, so lock it (door-flap letterbox, low horizon).

**NARR (draft):** "Bob gets three reactions, all day, every day. Ignore.
Politely reject — which is ignoring with a sorry attached. Or engage. Have a
guess which one's rarest." — the audience should recognise *their own* reaction.

## Scene 7 — therapy

**Shot:** A tasteful consulting room. Tarquin on the couch, genuinely baffled.
Play it for comedy.

**TARQUIN (draft):** "The other night I drove past some homeless people and I
laughed — obviously — and then I felt… something. What *is* this feeling?"

**NARR (draft):** "The first guilt of Tarquin's entire life. Forty-seven years.
Personal best."

**THERAPIST (draft — the punchline, keep it):** "I don't think there's any
amount of therapy that can help you. You need the most powerful psychedelic
known to man."

## Scene 8 — the standoff

**Shots:** The wank tank swings into the Waitrose car park — right beside
Bob's tent *(Easter egg: straddling two bays — "Two parking spaces. For
that.")*. The two men lock eyes. **One short beat of judgement each — no
prolonging** — then Tarquin strides into the shop / drives off.

**TARQUIN (draft):** "Get a job, you lazy c***."
**BOB (draft):** "F***ing wank tanks. You're all pricks. …I wish my wife was
here."

The crack in the armour from Scene 5 slaps shut. Cut.

## Scene 9 — the trip

*(The forest-drive scene is dropped — do not shoot it.)*

**Shots:**
1. The Welsh retreat, night: dome tents by a river, a small ceremony fire.
   Keep it minimal — the shaman *(name option: Moonwhisper Ascending)* says her
   piece; Tarquin grabs his knee.
2. Cut straight to the kick-in.
3. The water: Tarquin's reflection. His **eyes close with the scroll/cut…**

**TARQUIN (draft):** "F*** me, that kicked in quick."

**NARR (draft, over the reflection):** "Tarquin has a profound moment about the
fragility of human existence. That the distance between him and that man
outside the supermarket is thinner than he thought." — **No AI mention here.**
The vision is *wind your neck in, d***head* — not a preview. The newspaper does
the reveal later.

## Scene 10 — the wake-up

**Shots:**
1. **Eyes open — exactly Bob's POV from Scene 6:** inside the tent, the
   door-flap letterbox, the car park beyond. Same framing, locked.
2. But outside: chaos. Tents, wreckage, frost — and **a fire burning (possibly
   the only animated element in the scene)**.
3. Tarquin crawls out into the ruined Waitrose car park. *(Easter eggs: the
   W-AI-trose sign with only the A and I still lit; the wank tank still there —
   dead, filthy, clamped.)*
4. There's Bob.

**NARR (draft — the narrator does the freak-out, and still thinks it's the
drugs):** "What the f*** is going on. That… must be really strong stuff."

**TARQUIN (draft):** "Bad trip. This is a bad trip. I don't like it. I'm going
to get in my car and go home."

## Scene 11 — the newspaper

**Shots:** Face to face. Recognition. Bob picks up / points to the newspaper
and hands it over — **one prop, two jobs: the date (five years gone) and the
headline (AI TAKES THE JOBS — his included).** Tarquin reads. Hold.

**TARQUIN (draft):** "I remember you. I saw you just… yesterday."
**BOB (draft):** "No no no, mate. That was like five years ago. Here — look at
the date."
**BOB (draft, the man-sized empathy):** "Where the f*** have you been, then?
You look in a worse state than when I saw you last. You alright?"

**NARR (draft):** "It's not the drugs." — the moment the trip stops being
deniable, for Tarquin and narrator both. The five missing years are **never
explained**. Bob asks *him*.

## Scene 12 — the ending: embers into bad code

**Shots:**
1. The two of them sat by the fire, in the car park (**not** the woods). Their
   faces changed — almost a smile, almost glad of each other.
2. The man-way apology: cans clinked. Nothing said straight.
3. It's cold. **The newspaper goes on the fire.** It burns.
4. Camera pans up with the smoke — **the rising embers resolve into the bad
   code.** Hold. End.

**NARR (draft, closing):** "Well. We were both wrong, weren't we." *(Final line
open — land the shared-humans note without going soft; see the decisions log
above.)*

*(Optional post-credits stinger: an ATM screen — `EMPLOYMENT COLLAPSE SURVIVAL
MENU / ▸ STAY PUT` — "THANK YOU FOR BANKING WITH US.")*

## Scene → asset checklist

| Scene | Needs | Reference |
| --- | --- | --- |
| 1 | NatWest Tower skyline + "2008" (**keep the plate**); young-Tarquin variant; background newspaper reader, face behind the paper | `characters/tarquin.md` (young variant note) |
| 2 | Ordinary car, Bob + wife | `characters/bob.md` |
| 3 | The crash (wordless) | sound design only |
| 4 | Shard skyline + "2026" — **edited from the Scene 1 plate**; Shard reference image; Tarquin at the top | mirror Scene 1 framing |
| 5 | Night drive, homeless streets, the face-drop hold | `characters/wank-tank.md` |
| 6 | Waitrose car park, tent, tent-POV framing (lock it) | `characters/tent.md`, `style.md` |
| 7 | Consulting room | — |
| 8 | Car park standoff, two-bay parking | scene 6 location |
| 9 | Welsh retreat, ceremony fire, water reflection, blink | `style.md` (warmth rules) |
| 10 | Ruined car park, tent-POV rhyme (same framing as 6), fire | `characters/tent.md` |
| 11 | The newspaper prop (date + AI headline) — callback to the Scene 1 plant | scene 1 |
| 12 | Fire, newspaper burn, ember→bad-code rise | `style.md` |
