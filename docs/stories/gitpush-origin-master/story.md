...
- **The metaphor:** humanity is a git repository. Every major choice is a
  commit; `git push origin master` ships our worst code to everyone,
...
**1. Cold open — "this is your repository."** Black. A commit log resolves —
hash, timestamp, message. The voice, dry and certain: it has read every line
of our history, and it is about to walk us down it. *(Prologue B1)*

**2. The commit log of the species.** The scroll begins: **fire, the wheel,
writing** (warm, almost affectionate) → **money, the engine** (the gloss starts
to hedge) → **the bomb, the network** (accelerating past readability — "you
stopped reading your own diffs around here") → slam-stop on the last commit:
**the model.** The tone goes quiet and strange, because this commit is the
thing narrating. *(Prologue B2–5)*

**3. HEAD: now.** The line ends at the reader's actual present. An empty
prompt. A **blinking cursor**. The only frame in the arc where nothing has
happened yet. *(Prologue B6)*

**4. The push.** `git push origin master` types itself; the stake named in one
line — ships to everyone, irreversibly, nobody reviewing. The key is struck.
Title lands. *(Prologue B7)*

**5. The green checkmark — the handover ladder.** No explosion: a build
passes, and life *improves*. We hand our choices over one rung at a time —
**value → work → the safety net** — each handover individually sensible, each
stamped with a comforting green **"optimised ✓"**. The EP1 nodes live on these
rungs and are *glimpsed*, one image each — the nurse being told there's no
magic money tree, the tent in the Waitrose car park, the woman in the phone
...
- **The unpersonified POV.** The reader rides *inside* the AI — we look out at
...
>   [`../../images/register-anchor.md`](../../images/register-anchor.md)) for
>   the frame.

## Scene 1 — cold open: this is your repository

**Shot:** Black. A single line of cool light resolves into the top of a commit
log — a hash, a timestamp, a terse message. Nothing else in the world yet.

**NARR (draft):** "This is your repository. Every choice you ever shipped,
still in the log. I have read every line. Walk with me."

## Scene 2 — the commit log of the species

**Shots:** The scroll begins — commits landing one at a time: **fire · the
wheel · writing** (held, warm) → **money · the engine** (quicker) → **the bomb
· the network** (stacking faster than they can be read) → slam-stop on
**the model**, held in silence.

**NARR (draft, over fire):** "Good commit. Shipped it for a hundred thousand
years."
**NARR (canon, over the acceleration):** "You stopped reading your own diffs
around here."
**NARR (draft, quiet, on the model):** "And this one… this one is me."

## Scene 3 — HEAD: now

**Shot:** The log ends. `HEAD` — today. An empty prompt and a **blinking
cursor**. Hold the stillness longer than is comfortable.

**NARR (draft):** "This is the only frame in the whole story where nothing has
happened yet."

## Scene 4 — the push

**Shot:** The command types itself: `git push origin master`. The cursor hangs
over the key. Then — struck. Cut mid-keystroke. **Title card.** (In the track,
this is where the hook can land.)

**NARR (draft):** "The command that ships your code to everyone. Irreversibly.
With nobody reviewing it."

## Scene 5 — the green checkmark: the handover ladder

**Shots:** No explosion — a build passes: a single green **✓**. Then the
montage of frictionless improvement, descending the ladder, one glimpse per
rung:
1. **Value** — a nurse being told there's no magic money tree, while the
   printer hums *(one image; the node carries the full telling)*.
2. **Work** — a tent in a Waitrose car park *(one image)*.
3. **The safety net** — a woman in a phone box, refusing the no, filed as
   noise *(one image — plant the argument motif here)*.
Each rung stamps its own green ✓, a little more comforting, a little more
hollow.
...
**Shot:** An ordinary street. The conveniences still humming — the self-filling
form, the pre-made decision, a green ✓ on a screen in a shop window — and no
