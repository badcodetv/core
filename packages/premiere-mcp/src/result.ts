 ...
 * array, so a list-returning tool that set it would fail with an opaque "expected record,
 * received array" before its result ever reached the caller. The array is not wrapped in a
 * synthetic key here because `content.text` already carries the payload verbatim; inventing
 * `{ items: [...] }` would make the two encodings of the same result disagree.
 */
